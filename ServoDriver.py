...
class ServoDriver:
    ...
    def getPosition(self, servo):
        return self.servoAngles[servo]

    def setPosition(self, servo, degrees):
        self.__setPosition(servo, degrees)
        
    def setPositionSmooth(self, servo, degrees, delay=0.01, step=1):
        current_angle = self.getPosition(servo)
        if degrees < current_angle:
            for angle in range(current_angle, degrees - 1, -step):
                self.__setPosition(servo, angle)
                sleep(delay)
        else:
            for angle in range(current_angle, degrees + 1, step):
                self.__setPosition(servo, angle)
    ...
    def __calcPulseLength(self,degrees):
        return int(degrees*self.degreesToUS + 500)

    # setPosition takes a degree position for the servo to goto. 
    ...
    def __setPosition(self,servo, degrees):
        pulseLength = self.__calcPulseLength(degrees)
        self.__goToPeriod(servo,pulseLength)
        self.servoAngles[servo] = degrees
    
    def __goToPeriod(self,servo, period):
        if(period < 500):
            period = 500
        if(period >2500):
            period =2500
    ...
    def __init__(self, numberOfServos = 8, initialAngle = 90):
        servoPins = [2,3,4,5,6,7,8,9]
        self.servos = []
        self.servoAngles = [0] * numberOfServos  # Initialize empty list for servo angles
        ...
        for i in range(numberOfServos):
            ...
            #self.servos[i].put(1500)
            #self.servoAngles[i] = 90
            self.registerServo(i)
            self.__setPosition(i,initialAngle)
            #self.servos[i].put(1500) # Change this between 500 to 2500 for 0 to 180 degrees initial position
            #self.servoAngles[i] = 90  # Initialize the angle of current servo to 90 degrees
